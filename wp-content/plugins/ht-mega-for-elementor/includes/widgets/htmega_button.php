...
class HTMega_Elementor_Widget_Button extends Widget_Base {
    ...
    protected function render( $instance = [] ) {
        ...
        if( !empty( $settings['button_icon']['value'] ) ){
            $button_text = sprintf( '<a %1$s><span class="htmega_button_txt">%2$s</span><span class="htmega_button_icon">%3$s</span></a>', $this->get_render_attribute_string( 'url' ), $settings['button_text'], $button_icon );
        }
        if( !empty( $button_text ) ){
            echo sprintf( '<div %1$s>%2$s</div>', $this->get_render_attribute_string( 'htmega_button' ), $button_text );
        }
    }
}
